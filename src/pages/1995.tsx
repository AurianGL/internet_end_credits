...
export const NinetyFive: React.FC<HomeProps> = () => {
  ...
  return (
    <div className={`flex justify-start align-start fixed text-center ${mode === 'ANTHUME' ? 'bg-windows-700' : 'bg-gray-900'} w-screen`}>
      <Welcome />
      <div className="w-full h-full absolute z-50 bg-scanline bg-[length:4px_4px] pointer-events-none"></div>
      <div className="w-full h-full absolute z-50 bg-vignette pointer-events-none"></div>
      {mode === "POSTHUME" &&
      <div className={`${window.innerWidth > 500 ? 'animate-static' : ''} w-full h-full absolute z-50 bg-noise pointer-events-none bg-blend-difference opacity-20`}
      style={{
